...
if (window.innerWidth <= 800) {
  ...
  const mHeader = document.querySelector('.m-header .m-logo');
  ...
  logo.style.transition = 'top 1.2s cubic-bezier(0.4,0,0.2,1), left 1.2s cubic-bezier(0.4,0,0.2,1), transform 1.2s cubic-bezier(0.4,0,0.2,1)';
  logo.style.zIndex = '9001';
  logo.style.cursor = 'pointer';
  logo.onclick = mGoHome;

  // 1200ms 후: 헤더 위치(좌측 상단)로 이동
  setTimeout(() => {
    logo.style.top = '14px';
  ...
  // 1800ms 후: 라인 애니메이션
  setTimeout(() => {
    document.getElementById('m-intro-line').classList.add('open');
  }, 1800);

  // 2800ms 후: 오버레이 제거, 로고는 그 자리에 남아 헤더 역할
  setTimeout(() => {
    overlay.classList.add('done');
    // transition 제거해서 이후엔 딱딱하게 고정
